...
export const PartsInventoryPanel = ({ partsInventory, groupOptions, onSave }) => {
  ...
  const partsInventoryGroups = useSelector(
    selectGroupsByIdsAsSelectValues(partsInventory.groups || [])
  );

  const [groups, setGroups] = useState(partsInventory.groups || []);

  // state to know which group fields has the user selected
  const [currentGroupSelections, setCurrentGroupSelections] = useState(partsInventoryGroups);

  const onSavePartsInventory = () => {
    const newPartsInventory = {
      ...partsInventory,
